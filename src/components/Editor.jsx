...
const Editor = ({ 
  ...
  return (
    <div className="flex-1 flex flex-col h-full relative transition-colors duration-300 bg-white min-w-0 min-h-0">
      ...
      {/* Editor Content Area */}
      <div className="flex-1 w-full flex flex-col min-h-0 relative bg-white">
        {mode === 'text' ? (
          <textarea
            ref={textareaRef}
            value={markdown}
            ...
            className="flex-1 w-full h-full resize-none focus:outline-none leading-relaxed transition-colors duration-300 bg-white text-darkGray p-4 md:p-6 overflow-y-auto"
            style={{ 
              fontSize: fontSize, 
      ...
      {/* Floating Indicator */}
      {mode === 'text' && (
        <div className="absolute right-6 top-14 px-2 py-0.5 text-[8px] font-black uppercase tracking-widest rounded-full border border-mediumGray bg-lightGray text-darkGray opacity-40 transition-all select-none pointer-events-none hidden md:block z-10">
          UTF-8 • MD
        </div>
      )}
      ...
      {mode === 'text' && (
        <div className="h-10 border-t px-2 md:px-4 flex items-center font-semibold text-[9px] uppercase tracking-widest bg-[#F9F9F9] border-lightGray text-darkGray overflow-x-auto no-scrollbar whitespace-nowrap shrink-0 z-10 w-full shadow-[0px_-2px_10px_rgba(0,0,0,0.03)] pb-1">
